...
connection = mysql.connector.connect(host=cr.host,user=cr.user,password=cr.password)
curs1 = connection.cursor()
#lib1 = "DROP DATABASE lib"
lib2 = "CREATE DATABASE lib"
#curs1.execute(lib1)
curs1.execute(lib2)
connection.commit()

connection1 = mysql.connector.connect(host=cr.host,user=cr.user,password=cr.password,database=cr.database)
curs = connection1.cursor()

curs.execute("CREATE TABLE book_list (book_id INT PRIMARY KEY,book_name VARCHAR(255),author VARCHAR(255),edition VARCHAR(255),price FLOAT, qty INT);")

connection1.commit()

curs.execute("CREATE TABLE borrow_record (book_id INT,book_name VARCHAR(255),stu_roll VARCHAR(255),student_name VARCHAR(255),course VARCHAR(255),subject VARCHAR(255),issue_date DATE,return_date DATE,FOREIGN KEY (book_id) REFERENCES book_list(book_id));")
...
class Management:
    ...
    def DeleteBook(self):
        ...
        try:
            ...
            if status == True:
                ...
                if len(var) != 0:
                    messagebox.showwarning("Critical Warning!", "You can't delete this book record because it has borrow records.")
                else:
    ...
    def ReturningBook(self):
        ...
        if not x:
            messagebox.showwarning("Warning", "Please select a record to return.", parent=self.window)
            return
        ...
        try:
            ...
            if status == True:
                ...
                if var:
                    ...
                    messagebox.showerror("Error!", "Book not found in the database!", parent=self.window)
    ...
    def IssueAgain(self):
        try:
            x = self.tree_1.selection()
            if not x:
                messagebox.showwarning("Warning", "Please select a record to issue again.", parent=self.window)
                return
            
            row = self.tree_1.item(x)['values']
        
            if not row:
                messagebox.showwarning("Warning", "Selected record has no data.", parent=self.window)
                return
            ...
            self.submit_bt = Button(self.frame_1, text='Submit', font=(cs.font_1, 12), bd=2,
                                     command=lambda: self.BorrowBookAgain(row), cursor="hand2", bg=cs.color_2,
    ...
    def UpdateBookDetails(self):
        ...
        self.submit_bt_1 = Button(self.frame_1, text='Submit', font=(cs.font_1, 12), bd=2, command=lambda: self.SubmitforUpdateBook(row), cursor="hand2", bg=cs.color_2, fg=cs.color_3).place(x=310, y=459, width=100)
    ...
    def ShowRecordsforReturn(self):
        if self.roll_no_entry.get() == "":
            ...
            try:
                ...
                if len(rows) == 0:
                    messagebox.showerror("Error!", "This roll no. doesn't exist", parent=self.window)
                    connection.close()
                    ...
                    for row in rows:
                        self.tree_1.insert("", 'end', text=(rows.index(row) + 1), values=row)
    ...
    def SearchBook(self):
        ...
        try:
            ...
            search_term = "%" + self.book_entry.get() + "%"
            sql = "SELECT * FROM book_list WHERE book_name LIKE %s"
            values = (search_term,)
            ...
            messagebox.showerror("General Error!", f"An unexpected error occurred: {e}", parent=self.window)
    ...
    def Submit(self):
        if not all([self.id_entry.get(), self.bookname_entry.get(), self.author_entry.get(), self.edition_entry.get(), self.price_entry.get(), self.qty_entry.get()]):
        ...
        try:
            ...
            try:
                ...
                messagebox.showerror("Error!", "Invalid input. ID and Quantity must be integers, Price must be a number.", parent=self.window)
                return

            
            sql = "INSERT INTO book_list (book_id, book_name, author, edition, price, qty) VALUES (%s, %s, %s, %s, %s, %s)"
            values = (book_id, self.bookname_entry.get(), self.author_entry.get(), self.edition_entry.get(), price, qty)
    ...
    def Submit_borrow_data(self):
        if not all([self.book_id_entry.get(), self.book_name_entry.get(), self.stu_roll_entry.get(), self.stu_name_entry.get(), self.issue_date_entry.get(), self.return_date_entry.get(), self.course_entry.get(), self.subject_entry.get()]):
            messagebox.showerror("Error!", "Sorry!, All fields are required", parent=self.window)
            return
        ...
        try:
            ...
            stu_roll = self.stu_roll_entry.get()
            book_name = self.book_name_entry.get()

            
            sql = "SELECT * FROM book_list WHERE book_id = %s"
            values = (book_id,)
            curs.execute(sql, values)
            row = curs.fetchone()

            if not row:
                messagebox.showerror("Error!", "This book doesn't exist", parent=self.window)
                connection.close()
                return
            ...
            if book_quantity <= 0:  
                messagebox.showwarning("Notification", "This book is currently out of stock.")
                connection.close()
                return

            
            sql = "SELECT * FROM borrow_record WHERE stu_roll = %s AND book_id = %s"
            values = (stu_roll, book_id)
            curs.execute(sql, values)
            row = curs.fetchone()

            if row:
                messagebox.showerror("Error!", "This book is already taken by this student", parent=self.window)
                connection.close()
                return

            
            sql = "SELECT COUNT(*) FROM borrow_record WHERE stu_roll = %s"
            values = (stu_roll,)
            curs.execute(sql, values)
            count = curs.fetchone()[0]

            if count >= 3:
                messagebox.showerror("Max. 3 books to each student", "Borrow books limit has exceeded", parent=self.window)
                connection.close()
                return

            
            data_to_insert = (book_id, book_name, stu_roll, self.stu_name_entry.get(),
                              self.course_entry.get(), self.subject_entry.get(), self.issue_date_entry.get(),
                              self.return_date_entry.get())
            sql = "INSERT INTO borrow_record (book_id, book_name, stu_roll, student_name, course, subject, issue_date, return_date) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"
            curs.execute(sql, data_to_insert)

            
            book_quantity -= 1
            sql = "UPDATE book_list SET qty = %s WHERE book_id = %s"
            values = (book_quantity, book_id)
            curs.execute(sql, values)
